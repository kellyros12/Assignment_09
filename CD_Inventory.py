...
# KRos, 2020-Mar-22, added track handling, and sub menu choices
# KRos, 2020-Mar-22, added line 46 so the program would run if a non existent cd was entered
#------------------------------------------#
    ...
    if strChoice == 'l':
        ...
        if cd is not None:
            ...
            if strChoice == 'a':
                ...
                IO.ScreenIO.show_tracks(cd)
                del_trackno = int(input('Which track number would you like deleted? '))
                cd.rmv_track(del_trackno)
            elif strChoice == 'x':
                print('Returning to main menu.')
                continue # goes back to former loop
            else:
                print('General Error')
            continue
    elif strChoice == 's':
